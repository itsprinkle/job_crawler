	...
	###在sub下面解析item
	   ...
	   item_pattern_list={}
		...
		item_pattern_list[item_name]=item_xpath
	     ...
	     node_pattern_list=[]
             node_pattern_dict[url][item_name]={}
	     for field in node_field:
	         node_xpath = get_attrvalue(field,'xpath')
	         node_name = get_attrvalue(field,'name')
                 if node_name in node_pattern_dict[url][item_name]:
                    if node_xpath == "":
		       node_pattern_dict[url][item_name][node_name].append("NULL")
	            else:
		       node_pattern_dict[url][item_name][node_name].append(node_xpath)
                 else:
		    node_pattern_dict[url][item_name][node_name]=[]
		    node_pattern_dict[url][item_name][node_name].append(node_xpath)
